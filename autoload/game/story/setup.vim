...
function! game#story#setup#cmd_frame(state, thread_ref, stage_name) abort
  ...
  let l:next_state = game#story#records#record_fact_for_thread(l:next_state, l:threads[l:idx - 1], l:opening)
...
function! game#story#setup#cmd_npc(state, subcmd, npc_name) abort
  ...
  if a:subcmd ==# 'add'
    ...
    let l:next_state = game#story#records#record_fact(l:next_state, a:npc_name . ' is present in the current scene.')
